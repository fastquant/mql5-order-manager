...
class COrderBook : public CExpertMoney{
   private:
      CList *m_orderList;
      IMarket *m_market;
...
bool COrderBook::Execute(COrder *order){
...
bool COrderBook::Close(COrder *order){
   CAccountInfo accountInfos;
...
COrderBook::COrderBook(IMarket *market){
...
//+------------------------------------------------------------------+
//| Get the number of tickets in the orderBook
//| @return The number of tickets
//+------------------------------------------------------------------+

int COrderBook::GetNbrOfTickets(){
...
COrderInfos* COrderBook::GetOrderInfos(uint position){
   if(position<(uint)m_orderList.Total()){
      return m_orderList.GetNodeAtIndex(position);
   }
   else{
      Print("COrderBook::GetOrderInfos: There is no order at position "+IntegerToString(position));
      return NULL;
   }
} 
...
bool COrderBook::CreateOrder(string symbol, ENUM_ORDER_TYPE orderType,
                              double volume, double stopLoss=0,
                              double takeProfit=0, double price = 0){
   ...
   if(!ticket.SetSymbol(symbol)){
      delete ticket;
      Print("COrderBook::CreateOrder: Symbol error, order creation aborted.");
      return false;
   }
   ...
   if(!ticket.SetPrice(price)){
      delete ticket;
      Print("COrderBook::CreateOrder: Price error, order creation aborted.");
      return false;
   }
   ...
   if(!ticket.SetVolume(volume)){
      delete ticket;
      Print("COrderBook::CreateOrder: Volume error, order creation aborted.");
      return false;
   }
   ...
   if(!ticket.SetTakeProfit(takeProfit)){
      delete ticket;
      Print("COrderBook::CreateOrder: TakeProfit error, order creation aborted.");
      return false;
   }
   ...
   if(!ticket.SetStopLoss(stopLoss)){
      delete ticket;
      Print("COrderBook::CreateOrder: StopLoss error, order creation aborted.");
      return false;
   }
   ...
   if(this.Execute(ticket)){
      ...
      delete ticket;
      Print("COrderBook::CreateOrder: Error during execution.");
      return false;
   }
}
...
bool COrderBook::CloseOrder(uint nbr){
   if(nbr<(uint)m_orderList.Total()){
      ...
      if(ticket.GetStatus()==ORDER_STATE_FILLED){ // If the order is executed
         if(!this.Close(ticket)){
            Print("COrderBook::CloseOrder: Impossible to close the order");
            return false;
         }
         else
            return true;
         ...
         Print("COrderBook::CloseOrder: The order is already closed");
         return false;
      }

   }
   else{
      Print("COrderBook::CloseOrder: There is no ticket at the position "+IntegerToString(nbr));
      return false;
   }
}
...
bool COrderBook::CloseOrders(){
   ...
   for(int i = 0; i < listSize; i++){
      if(ticket.GetStatus()==ORDER_STATE_FILLED){ // If the order is executed
         if(!this.Close(ticket)){
            Print("COrderBook::CloseOrders: Impossible to close the order");
            return false;
         }
         else
            Print("COrderBook::CloseOrders: Order "+IntegerToString(ticket.GetID())+" closed");
      }
...
//+------------------------------------------------------------------+
//| Synchronise orderBook with the market status
//| @return true if successfull, false otherwise
//+------------------------------------------------------------------+

bool COrderBook::Synchronise(){
   COrder *ticket;
   double ask, bid;
   int listSize, index;
   ...
   for(int i = 0; i < listSize; i++){
      ...
      if(ask==0.0||bid==0.0){
         Print("COrderBook::Synchronise: Recuperation of market prices impossible.");
         return false;
      }
      ...
      if(ticket.GetStatus()==ORDER_STATE_FILLED){
         if(ticket.GetType()==ORDER_TYPE_SELL){
            if((ticket.GetStopLoss()<=bid&&ticket.GetStopLoss()!=0.0)
               ...
               // Close the order
               if(!this.Close(ticket)){
                  Print("COrderBook::Synchronise: Error during closing order "+IntegerToString(ticket.GetID()));
                  return false;
               }
               else
                  Print("COrderBook::Synchronise: Order "+IntegerToString(ticket.GetID())+" closed");
            }
         }
